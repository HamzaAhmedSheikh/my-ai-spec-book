...
class ChatQueryRequest(BaseModel):
    ...
    top_k: int = Field(
        default=5, ge=1, le=10, description="Number of relevant chunks to retrieve"
    )
...
class ChatQueryResponse(BaseModel):
    ...
    sources: List[SourceCitation] = Field(
        ..., min_length=1, max_length=10, description="List of source citations"
    )
...
class ErrorResponse(BaseModel):
    ...
    error: str = Field(
        ..., description="Error type (e.g., 'validation_error', 'rate_limit')"
    )
